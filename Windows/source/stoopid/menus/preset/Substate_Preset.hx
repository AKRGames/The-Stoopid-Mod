...
class Substate_Preset extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		FlxTween.tween(blackBarThingie, {'scale.y': 230}, 0.5, {ease: FlxEase.circOut});
			...
			if (controls.ACCEPT)
			{
				switch (optionShit[curSelected])
				{
					case 'clear':
						...
						FlxTween.tween(blackBarThingie, {'scale.y': 1500}, 0.5, {ease: FlxEase.expoOut});
						FlxG.camera.fade(FlxColor.BLACK, 0.42);
		...
		menuItems.forEach(function(spr:FlxSprite)
		{
			if (!goingBack)
			{
				...
				if (spr.ID == curSelected)
					spr.scale.set(FlxMath.lerp(spr.scale.x, 1.5, camLerp / (_variables.fps / 60)),
						FlxMath.lerp(spr.scale.y, 1.5, 0.4 / (_variables.fps / 60)));
			}
			else
