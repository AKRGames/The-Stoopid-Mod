package stoopid.backend.system.haxelibrewrites;
...
/**
 * `OldAhhFlxGame` is the heart of all Flixel games, and contains a bunch of basic game loops and things.
 * It is a long and sloppy file that you shouldn't have to worry about too much!
 * It is basically only used to create your game object in the first place,
 * after that `FlxG` and `FlxState` have all the useful stuff you actually need.
 */
@:allow(flixel.FlxG)
class OldAhhFlxGame extends Sprite
{
	/**
	...
	#if FLX_SOUND_TRAY
	/**
	 * Change this after calling `super()` in the `OldAhhFlxGame` constructor
	 ...
	 * @param Zoom            The default level of zoom for the game's cameras (e.g. `2` = all pixels are now drawn at 2x). Fun fact, this was removed but we brought it back because Mic'd Up needs it frfr
